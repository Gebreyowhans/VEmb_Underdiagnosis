...
def Subgroup_FPR(df, diseases, category, category_name, seed=19, fpr_path="default"):
    ...
    for i in range(len(FN_total)):
        ...
        if category_name == 'insurance':
            ...
            if i == 2:
                ...
                FPR_Ins.to_csv(fpr_path+"run_"+str(seed) +
        ...
        if category_name == 'race':
            ...
            if i == 5:
                ...
                FPR_race = pd.concat(
                    [FPR_race, FNR_A.reindex(FPR_race.index)], axis=1)
                FPR_race.to_csv(fpr_path+"run_" +
                                str(seed)+"FPR_FNR_NF_race.csv")


def Two_Group_FPR(df, diseases, category1, category_name1, category2,category_name2, seed=19, fpr_path_2_group_intersection="default"):
    ...
    if (category_name1 == 'gender') & (category_name2 == 'insurance'):
        FP_InsSex.to_csv(fpr_path_2_group_intersection +
                         "run_"+str(seed)+"FP_InsSex.csv")

    if (category_name1 == 'gender') & (category_name2 == 'race'):
        FP_RaceSex.to_csv(fpr_path_2_group_intersection +
                          "run_"+str(seed)+"FP_RaceSex.csv")

    if (category_name1 == 'insurance') & (category_name2 == 'race'):
        FP_InsRace.to_csv(fpr_path_2_group_intersection +
                          "run_"+str(seed)+"FP_InsRace.csv")

    if (category_name1 == 'insurance') & (category_name2 == 'age_decile'):
        FP_InsAge.to_csv(fpr_path_2_group_intersection +
                         "run_"+str(seed)+"FP_InsAge.csv")

    if (category_name1 == 'race') & (category_name2 == 'age_decile'):
        FP_RaceAge.to_csv(fpr_path_2_group_intersection +
                          "run_"+str(seed)+"FP_RaceAge.csv")

    if (category_name1 == 'gender') & (category_name2 == 'age_decile'):
        FP_AgeSex.to_csv(fpr_path_2_group_intersection +
...
def main():
    ...
    for seed in seeds:
        ...
        fpr_path = "./FPR/SubGroup_FPR/"
        os.makedirs(os.path.dirname(fpr_path), exist_ok=True)
        ...
        for i in range(len(factor)):
            Subgroup_FPR(df, [str(diseases[13])], factor[i],
                           factor_str[i], seed, fpr_path)

        '''' Two group FPR '''
        fpr_path_2_group_intersection = "./FPR/Two_Group_Intersection_FPR/"
        os.makedirs(os.path.dirname(
            fpr_path_2_group_intersection), exist_ok=True)

        Two_Group_FPR(
            df, [str(diseases[13])], gender, 'gender', race, 'race', seed, fpr_path_2_group_intersection)
        Two_Group_FPR(df, [str(diseases[13])], gender, 'gender', age_decile,
                                    'age_decile', seed, fpr_path_2_group_intersection)
        Two_Group_FPR(df, [str(diseases[13])], race, 'race', age_decile,
                                    'age_decile', seed, fpr_path_2_group_intersection)
        Two_Group_FPR(df, [str(diseases[13])], insurance, 'insurance',
                                    age_decile, 'age_decile', seed, fpr_path_2_group_intersection)
        Two_Group_FPR(df, [str(diseases[13])], insurance, 'insurance',
                                    race, 'race', seed, fpr_path_2_group_intersection)
        Two_Group_FPR(df, [str(diseases[13])], gender, 'gender',
                                    insurance, 'insurance', seed, fpr_path_2_group_intersection)

        '''' Three group FPR '''
        fpr_path_3_group_intersection = "./FPR/Three_Group_Intersection_FPR/"
        os.makedirs(os.path.dirname(
            fpr_path_3_group_intersection), exist_ok=True)

        Three_Group_FPR(df, [str(diseases[13])], gender, 'gender', age_decile,
                                      'age_decile', race, 'race', seed, fpr_path_3_group_intersection)
        Three_Group_FPR(df, [str(diseases[13])], insurance, 'insurance', gender,
                                      'gender', race, 'race', seed, fpr_path_3_group_intersection)
        Three_Group_FPR(df, [str(diseases[13])], insurance, 'insurance', gender,
                                      'gender', age_decile, 'age_decile', seed, fpr_path_3_group_intersection)
