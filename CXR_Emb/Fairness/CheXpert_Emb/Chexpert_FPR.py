...
def main():
  ...
  for seed in seeds:
    ...
    for i in range(len(factor)):
        Subgroup_FPR(df,diseases,factor[i],factor_str[i],seed,fpr_path)

    '''' Two group FPR '''
    fpr_path_2_group_intersection = "./FPR/Two_Group_Intersection_FPR/"
    os.makedirs(os.path.dirname(fpr_path_2_group_intersection), exist_ok=True)
    
    Two_Group_FPR(df, diseases, gender, 'gender',race,'race',seed,fpr_path_2_group_intersection)
    Two_Group_FPR(df, diseases, gender, 'gender', age_decile, 'age_decile',seed,fpr_path_2_group_intersection)
    Two_Group_FPR(df, diseases, race, 'race', age_decile, 'age_decile',seed,fpr_path_2_group_intersection)

    '''' Three group FPR '''
    fpr_path_3_group_intersection = "./FPR/Three_Group_Intersection_FPR/"
    os.makedirs(os.path.dirname(fpr_path_3_group_intersection), exist_ok=True)

    Three_Group_FPR(df, diseases,gender,'gender',age_decile, 
                    'age_decile',race,'race',
                    seed,fpr_path_3_group_intersection)
