...
from dataset import make_chexpert_dataset
...
def get_callbacks(patience=5, fold=1, save_model_dir="./mimic-cxr/"):
...
def training_and_evaluation(seed,base_path,df_train, df_validate, df_test, labels_Columns, thersholds, debug,is_mimic=False):
    np.random.seed(seed)
    ...
    if is_mimic:
        ...
        training_data, validation_data, test_dataset = make_chexpert_dataset(
        df_train=df_train, df_validate=df_validate, df_test=df_test, labels_Columns=labels_Columns)
    ...
    history = model.fit(x=training_data.batch(params["batch_size"]).prefetch(tf.data.AUTOTUNE).cache(),
                        validation_data=validation_data.batch(
                            params["batch_size"]).cache(), callbacks=get_callbacks(patience=5, fold=1, save_model_dir=checkpoint_path), epochs=epoches)
